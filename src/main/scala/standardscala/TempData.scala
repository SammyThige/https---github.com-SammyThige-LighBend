...
object TempData {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //Days with the hottest temperature
    ...
    println(s"There are $rainyCount rainy days. Which is a percentage of ${rainyCount*100.0/data.length} of all days")

    //    val (rainySum,rainyCount2) = data.foldLeft(0.0->0) {case ((sum,cnt), td) => if(td.precip<1.0) (sum,cnt) else (sum+td.tmax,cnt+1)}
